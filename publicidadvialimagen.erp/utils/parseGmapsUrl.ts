...
export async function parseGmapsUrl(input: string): Promise<LatLng | null> {
  try {
    ...
    const atPattern = /@(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
    ...
    try {
      const url = new URL(text, "https://maps.google.com");
      const q = url.searchParams.get("q");
      if (q) {
        const qPattern = /(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
        const qMatch = qPattern.exec(q);
        if (qMatch) {
      ...
      console.log('URL parsing failed, trying direct patterns');
    }

    // Patrón 4: &ll=lat,lng
    // Ejemplo: https://www.google.com/maps?ll=-16.5000,-68.1500
    const llPattern = /[?&]ll=(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
    ...
    const centerPattern = /[?&]center=(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
    ...
    const directPattern = /(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
    ...
    const placePattern = /\/place\/[^\/]*\/@(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
    ...
    const dirPattern = /\/dir\/\/\/(-?\d{1,3}(?:\.\d+)?),\s*(-?\d{1,3}(?:\.\d+)?)/;
